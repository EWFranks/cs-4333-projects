...
/**
 * A server implementation of the talk interface that prints messages from the
 * talk client on
...
public class TalkServer implements BasicTalkInterface {
  ...
  /**
   * Performs asynchronous IO using polling. Should print messages from the client
   * on STDOUT and
   * print messages from STDIN to the client. Messages printed on STDOUT should be
   * prepended with
   * "[remote] ".
   */
  ...
  public String status() {
    ...
    // IP.toString();
    // String int2String = Integer.toString(port);
    // String ok = ("<" + IP + ">:<" + int2String + ">");
    // } catch (UnknownHostException bruh) {
  ...
  /**
   * Performs synchronous IO by blocking on input. Should print messages from the
   * client on STDOUT
   * and print messages from STDIN to the client. Messages printed on STDOUT
   * should be prepended
   * with "[remote] ".
   */
  public void syncIO() throws IOException {
    ...
    if (this.stdin.ready()) this.sockout.println(this.stdin.readLine());
  }
}
